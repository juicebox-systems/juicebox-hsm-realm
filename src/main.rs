...
use client::{Client, Realm, RecoverError};
use server::Server;
use types::{AuthToken, Pin, Policy, UserSecret};
...
async fn main() {
    ...
    client
        .register(
            ...
            &UserSecret(String::from("teyla21")),
    ...
    match client.recover(&Pin(String::from("1212"))).await {
        Err(RecoverError::BadPin) => { /* ok */ }
        result => panic!("Unexpected result from recover: {result:?}"),
    };

    println!("main: Starting recover with correct PIN (guess 2)");
    let secret = client
        .recover(&Pin(String::from("1234")))
        .await
        .expect("recover failed");
    println!("main: Recovered secret {:?}", secret.0);
    ...
    match client.recover(&Pin(String::from("1234"))).await {
