...
async fn leader_battle() {
    ...
    // Should be able to make a request via the LB fine.

    // TODO: this request works fine, however it causes the subsequent manual
    // requests to the agent to fail. The agent gets stuck in a stale proof
    // retry loop. That'll keep retrying until it hits the "too slow to make
    // progress" panic. This happens because the agent reads the latest log
    // entry which is now newer than the hsm knows about so it doesn't have
    // the root hash from the proof. Perhaps app_request should include the
    // log index that the read was based on? that way the hsm could spot that
    // some other agent is really the leader.
    ...
    for result in results {
        ...
        assert!(
            result.is_ok_and(|r| matches!(r, SecretsResponse::Register1(Register1Response::Ok)))
        );
    }
    // All but one of the agent should have stepped down due to the log precondition check.
    ...
    assert!(successes[0]
        .as_ref()
        .is_ok_and(|s| matches!(s, SecretsResponse::Delete(DeleteResponse::Ok))));
    for error in errors {
        assert_eq!("RpcError network error", error.unwrap_err().to_string());
    }
...
async fn make_app_request_to_agents(
    ...
    realm: &RealmResult,
    req: SecretsRequest,
) -> Vec<anyhow::Result<SecretsResponse>> {
    ...
    join_all(realm.agents.iter().map(|agent| async {
        ...
        match rpc::send(agent_client, agent, r).await {
            ...
            Ok(other_response) => Err(anyhow!("Not ok response: {other_response:?}")),
            Err(err) => Err(anyhow!("RpcError {}", err)),
        }
    }))
    .await
}
