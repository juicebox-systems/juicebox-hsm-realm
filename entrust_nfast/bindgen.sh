...
bindgen --use-core \
    ...
    --allowlist-var ".*CreateSEEWorld_Args.*|.*LoadBuffer.*|NFastApp_ConnectionFlags.*|Key_flags_.*|Command_flags.*|.*_(enum)?table|.*PermissionGroup.*|Act_.*_Details_.*|FileDevice.*|Cmd_GenerateKey.*_flags.*|NFKM_NKF_.*|StatInfo_flags_.*" \
    --allowlist-type "M_SEEInitStatus|NFKM_(Admin.*|LoadAdminKeysHandle)|M_KeyMgmtEntType|M_StatNodeTag" \
