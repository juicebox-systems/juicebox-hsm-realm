...
bindgen --use-core \
    ...
    --allowlist-var ".*CreateSEEWorld_Args.*|.*LoadBuffer.*|NFastApp_ConnectionFlags.*|Act_NVMemOpPerms_Details_perms.*|Command_flags.*|.*_(enum)?table||Act_OpPermissions_Details.*|.*PermissionGroup.*|Act_MakeBlob_Details_flags.*|Cmd_GenerateKey_Reply_flags.*|NFKM_DEFOPPERMS.*|NFKM_NKF_.*" \
